...
|---------|---------|------|-------|
| Web SPA (Vite) | `pnpm --filter web web --host` | 1924 | Standalone mode; use `web` script to skip builder precheck |
| Web + SSR | `pnpm dev` | 1924 (Next.js), 13333 (Vite) | Requires `photos-manifest.json` to exist |
| Backend | `pnpm --filter @afilmory/core dev` | 1841 | Requires PostgreSQL + Redis (use `docker compose up db redis`) |
| Dashboard | `pnpm --filter @afilmory/dashboard dev` | 5173 | Requires backend running |

### Key caveats

- **Empty manifest bootstrap**: The `generated-routes.ts` file in `apps/web/src/` is auto-generated by Vite's `vite-plugin-route-builder` on first dev server start. Type-check (`pnpm --filter web type-check`) will fail until this file exists. Start the Vite dev server once to generate it.
- **Builder precheck**: Both `pnpm --filter web dev` and `pnpm --filter web build` run the builder CLI as a precheck (`scripts/precheck.ts`), which requires S3 storage credentials. To skip the precheck in dev, use `pnpm --filter web web --host` instead. For builds, run `cd apps/web && npx vite build` directly.
- **Manifest file**: If `apps/web/src/data/photos-manifest.json` does not exist, create an empty one: `mkdir -p apps/web/src/data && echo '{"data":[],"cameras":[],"lenses":[]}' > apps/web/src/data/photos-manifest.json`. The symlink at `packages/data/src/photos-manifest.json` points to this file.
- **ESLint**: `pnpm lint` crashes with a pre-existing `@typescript-eslint/consistent-generic-constructors` rule error (compatibility issue between `eslint@10`, `@typescript-eslint@8.55`, and TypeScript 5.9). This is not caused by environment setup.
- **Code quality**: See `AGENTS.md` Commands section and `README.md` for standard lint/format/type-check/build commands.
