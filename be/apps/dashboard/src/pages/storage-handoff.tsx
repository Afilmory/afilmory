import { Button, Modal, Switch } from '@afilmory/ui'
import { ArrowLeft, CheckCircle2, CloudCog, Plus, ShieldCheck } from 'lucide-react'
...
export function Component() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setSecureAccessEnabled(next.values.photoStorageSecureAccess === 'true')
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      const result = await saveStorageHandoff({ activeProviderId, providers, secureAccessEnabled })
  ...
  return (
    <HandoffShell>
      <div className="space-y-6">
        ...
        <LinearBorderPanel className="space-y-4 p-5">
          <div className="flex items-start gap-3">
            <ShieldCheck className="text-accent mt-0.5 size-5" />
            <div className="flex-1">
              <h2 className="text-text text-sm font-semibold">{t('storage.handoff.security.title')}</h2>
              <p className="text-text-secondary mt-1 text-xs">{t('storage.handoff.security.description')}</p>
            </div>
            <Switch checked={secureAccessEnabled} onCheckedChange={setSecureAccessEnabled} />
          </div>
        </LinearBorderPanel>
