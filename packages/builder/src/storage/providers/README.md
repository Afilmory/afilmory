...
## 通用接口更新：StorageObject.meta

自 vNext 起，`StorageObject` 增加可选字段 `meta?: Record<string, unknown>`，用于承载与文件相关的附加元数据（例如来源标记、处理标志、外部索引 ID 等）。

- 各存储提供商可按需填充该字段；未使用时可忽略。
- 该字段不会影响文件读取与 URL 生成逻辑，仅作为扩展信息通道。
